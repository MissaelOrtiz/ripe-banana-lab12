...
describe('reviews routes', () => {
  ...
  it('gets all reviews via GET', async () => {
    ...
    const review1 = { rating: 5, ReviewerId: reviewer.id, review: 'It good', FilmId: be.id };
    const review2 = { rating: 1, ReviewerId: reviewer.id, review: 'Nevermind I actually watched the movie', FilmId: be.id };
  ...
  it('updates a review via PATCH', async () => {
    ...
    const review = await Review.create({ rating: 1, ReviewerId: reviewer.id, review: 'It good', FilmId: be.id });

    const res = await request(app)
      .patch(`/api/v1/reviews/${review.id}`)
      .send({ rating: 5 });

    expect(res.body).toEqual({  id: 1, rating: 5, ReviewerId: reviewer.id, review: 'It good', FilmId: be.id });
  });
